...
export function EstimateResponseActions({
  ...
  const disabledReason = disabled
    ? "This estimate can no longer be responded to."
    : terms && !acceptedTerms
  ...
  return (
    <div className="mt-6">
      <div aria-live="polite" role="status">
        {status === "accepted" && (
          <div className="rounded-2xl bg-success-soft px-4 py-3 text-center text-sm font-medium text-success">
            Estimate accepted. The service provider can now convert it into an invoice.
          </div>
        )}
        {status === "declined" && (
          <div className="rounded-2xl bg-line px-4 py-3 text-center text-sm text-muted">
        ...
        <>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <button
              ...
              aria-describedby={disabledReason ? "estimate-action-hint" : undefined}
          ...
          {disabledReason && (
            <p id="estimate-action-hint" className="mt-2 text-center text-xs text-muted">
              {disabledReason}
            </p>
          )}
        </>
